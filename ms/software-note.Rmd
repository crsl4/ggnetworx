...
\missing{maybe we want to remove the basic split and explicit network plots and only show the cool plots. we need to show here all the cool functionalities of ggtree but now on networks}

# Split network for the Neobatrachus genus 
...
\missing{(add here short description of whales)}.
...
p4 = ggevonet(whalesnet, layout = "slanted")+geom_tiplab()
```
```{r whalesnet, out.width='60%', fig.cap="Explicit network for whales", fig.pos='H', fig.align='center'}
p4
```
