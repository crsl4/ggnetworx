...
Recent years have seen an explosion of phylogenetic network inference methods (\missing{cite everybody}: yu2014, wen2018, zhang2018, \cite{Solis-Lemus2016, Huson2010, Gruenewald2013, Than2008, Yang2014}. These methods can be divided into two main classes: methods to reconstruct implicit (or split) networks, and methods to reconstruct explicit networks. Implicit networks are data-displayed objects that include two (or more) options for every uncertain split. For example, in figure \ref{fig:nettypes} right, there are two possible ways to resolve the uncertain split: 1) by considering the longer edges of the rectangle to be the correct split (making the two spiders on the bottom sister species) or 2) by considering the shorter edges of the rectangle to be the correct split (making the two spiders on the right sister species).
However, the input data (DNA sequences or gene trees) does not provide enough information to distinguish which one is the "correct" split. Or alternatively, both splits are "correct" in that there is a pattern of gene flow among these species. Split networks are fast to reconstruct, but they are limited in their interpretations are they cannot tell us if the discordant splits are due to gene tree estimation error, ILS or actual gene flow.
Explicit networks, on the contrary, assign a biological mechanism to every internal node, and thus they are easier to interpret. For example, in figure \ref{fig:nettypes} left, the hybrid node (where the green arrow is pointing) corresponds to some reticulation event, and the tree nodes correspond to speciation events. Explicit networks are still limited in that they cannot identify the specific biological mechanism behind a hybrid node (hybridization, horizontal gene transfer, introgression), but hybrid edges are parametrized by an inheritance probability ($\gamma$) that can provide more information. For example, an estimated $\hat{\gamma} \approx 0.5$ could indicate a hybridization event, whereas smaller estimated $\hat{\gamma} \approx 0.01$ could refer to horizontal gene transfer. The downside of explicit networks is that they are harder to reconstruct, and existing inference methods\cite{Yu2014, Wen2018, Zhang2018, Solis-Lemus2016} are not scalable enough to handle big data.

```{r nettypes, out.width='35%', fig.cap="\\textbf{Left:} Explicit network. \\textbf{Right:} Split network.",fig.show='hold', fig.align='center'}
knitr::include_graphics(c('figures/explicit.pdf', 'figures/implicit.pdf'))
```
...
```{r netNewick, out.width='35%', fig.cap="Extended Newick format for networks: (((A,(B)\\#H1),(C,\\#H1)),D); The network is written as a tree with two nodes having the same label \\#H1.",fig.show='hold', fig.align='center', fig.pos='H'}
...
The `ggsplitnet` function takes a `networkx` object as input, and produces a plot (see Figure \ref{fig:enet} left). 
...
p = ggsplitnet(nnet) + geom_tiplab2(cex=2.5)
```
...
The `evonet` object has two matrices: 1) standard edge matrix with two columns for the two nodes connecting every edge (row), and 2) hybrid edges matrix with two columns for the nodes connected by the hybrid edge (row). See Figure \ref{fig:enet} right for the plot. 
...
```{r enet, out.width='50%', fig.cap="\\textbf{Left:} Split network for yeast data. \\textbf{Right:} Explicit network from extended Newick format", fig.pos='H', fig.align='center'}
library(cowplot)
plot_grid(p,NULL,p2,nrow=1, rel_widths=c(1,0.2,0.8))
```
...
```{r frognet, out.width='60%', fig.cap="Split network for Neobatrachus genus frogs", fig.pos='H', fig.align='center'}
p3
```
...
library(ggimage)
library(viridis)
whalesnet = read.evonet(text="(((bowhead,north-atlantic)eleven:9.055782863234407)#H10:2.6340479153169807::0.8331903627712247,(minke,((sei:9.41949690722463,(blue,#H18:0.0::0.013010091110208823)fifteen)fourteen:1.6484045144986545,((#H10:0.19346605673125924::0.16680963722877526,(gray:1.7595289284120899)#H18:8.238071084864815::0.9869899088897912)seventeen:0.35164828246609625,(humpback,fin)nineteen:2.146642742731017)sixteen:0.066043783465627)thirteen:1.100383484076166)twelve:2.6340479153169807)nine;")
label <- whalesnet$tip.label

p <- ggevonet(whalesnet) 

dat = read.csv("data-analysis/internal-nodes-traits.csv")
dat$nodenum = NULL

## We need to add two more rows because nodes 15 and 17 are repeated in p$data
dat2 = data.frame(label=p$data$label, trait = c(dat$trait,dat$trait[15], dat$trait[17]))

## Now we merge dataframes:
p2 <- p %<+% dat2
##p2$data
...
p3 <- p2 + 
    ...
    geom_tippoint(aes(size=trait, color=trait)) +
    geom_nodepoint(aes(size=trait, color=trait)) + 
    theme(legend.position = "none") + scale_size_continuous(range = c(3, 10))
...
p5 <- p2 + 
     xlim(0,8) + ylim(0,15) +
    geom_tiplab(geom='label', offset=0.75) +
    geom_tiplab(aes(image=paste0('figures/',label, '.png')), geom="image",
                offset=0.2) +
    geom_tippoint(aes(size=trait, color=trait)) +
    geom_nodepoint(aes(size=trait, color=trait)) + 
    theme(legend.position = "none") + scale_size_continuous(range = c(2, 6)) +
    scale_colour_viridis_c(begin=0.25, end=1)
```
```{r whalesnet, out.width='85%', fig.cap="Explicit network for whales: ancestral state reconstruction of population size: color and size", fig.pos='H', fig.align='center'}
p5
```
...
Furthermore, `ggtree` and thus `tanggle` are based on underlying data frames to represent the phylogenies, which allow a lot of flexibility on things mapped and colors and everything.
